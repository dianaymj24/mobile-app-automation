...
__export(api_exports, {
  ...
  addGlobalAttachment: () => addGlobalAttachment,
  addGlobalAttachmentFromPath: () => addGlobalAttachmentFromPath,
...
async function addGlobalError(details) {
  await (0, import_allure_js_commons.globalError)(details);
}
async function addGlobalAttachment(name, content, contentType = import_allure_js_commons.ContentType.TEXT) {
  const buf = Buffer.isBuffer(content) ? content : Buffer.from(content, "utf8");
  await (0, import_allure_js_commons.globalAttachment)(name, buf, { contentType });
}
async function addGlobalAttachmentFromPath(name, path, contentType = import_allure_js_commons.ContentType.TEXT) {
  await (0, import_allure_js_commons.globalAttachmentPath)(name, path, { contentType });
}

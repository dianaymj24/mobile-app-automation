...
var AllureReportState = class {
  ...
  async processRuntimeMessage() {
    for (let i = 0; i < this.messages.length; i++) {
      ...
      if (message.type === "global_error" || message.type === "global_attachment_content" || message.type === "global_attachment_path") {
...
async function addGlobalError(details) {
  await allureGlobalError(details);
}
async function addGlobalAttachment(name, content, contentType = ContentType.TEXT) {
  const buf = Buffer.isBuffer(content) ? content : Buffer.from(content, "utf8");
  await allureGlobalAttachment(name, buf, { contentType });
}
async function addGlobalAttachmentFromPath(name, path4, contentType = ContentType.TEXT) {
  await allureGlobalAttachmentPath(name, path4, { contentType });
}
...
function splitTitlePathPart(part) {
  if (!part) {
    return [];
  }
  return part.replace(/\\/g, "/").split("/").map((p) => p.trim()).filter(Boolean);
}
var AllureReporter = class _AllureReporter extends WDIOReporter {
  ...
  _titlePathFileByCid = /* @__PURE__ */ new Map();
...
${this._consoleOutput}`, "utf8"),
  ...
  _registerListeners() {
    ...
    process3.on(events.addTestInfo, (payload) => {
      ...
      if (/\.feature$/i.test(fileStr)) {
        ...
        const titlePath = this._titlePath(cid, Array.isArray(testPath) ? testPath.slice(0, -1) : void 0);
  ...
  onTestStart(test) {
    ...
    this._pushRuntimeMessage({
      type: "allure:test:info",
      data: { fullName, titlePath: this._titlePath(cid) }
    });
    const suitePath = [...this._suiteStack(cid)];

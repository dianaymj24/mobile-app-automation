...
function composeElementOverrides(previousCommand, nextCommand) {
  if (!previousCommand) {
    return nextCommand;
  }
  const previousElementCommand = previousCommand;
  return function composedElementOverride(originalCommand, ...args) {
    const element = this;
    function previousCommandAsOriginal(...previousArgs) {
      const context = this || element;
      function originalForPrevious(...originalArgs) {
...
function setElementOverride(overrides, name, command) {
...
var REJECTED = /* @__PURE__ */ Symbol("rejected");
...
var filter = (array, callback, thisArg) => {
  return new Promise((resolve, reject) => {
    ...
    return Promise.all(results).then((result) => result.filter((val) => val !== REJECTED)).then(resolve, reject);
  });
};
...
var testFrameworkFnWrapper = async function({ executeHooksWithArgs: executeHooksWithArgs2, executeAsync: executeAsync2 }, type, { specFn, specFnArgs }, { beforeFn, beforeFnArgs }, { afterFn, afterFnArgs }, cid, repeatTest = 0, hookName, timeout) {
  ...
  const identitySnapshot = typeof afterFnArgs === "function" ? afterFnArgs(this) : void 0;
  let result;
  let error;
  let skip = false;
  let autoSkipError;
  ...
  const afterArgs = identitySnapshot && identitySnapshot.length > 0 ? [identitySnapshot[0], ...liveAfterArgs.slice(1)] : liveAfterArgs;
  afterArgs.push({
    retries,
    error,
    result,

...
var DEFAULT_RESPONSE_TIMEOUT = 1e3 * 180;
var _id, _ws, _waitForConnected, _resolveWaitForConnected, _webSocketUrl, _clientOptions, _responseTimeout, _pendingCommands, _BidiCore_instances, handleResponse_fn;
var BidiCore = class {
  constructor(webSocketUrl, opts, responseTimeout = DEFAULT_RESPONSE_TIMEOUT) {
    ...
    __privateAdd(this, _responseTimeout);
    ...
    __privateSet(this, _responseTimeout, responseTimeout);
  ...
  async send(params) {
    ...
    const payload = await new Promise((resolve, reject) => {
      const t = setTimeout(() => {
        reject(new Error(
          "Command ".concat(params.method, " with id ").concat(id, " timed out after ").concat(__privateGet(this, _responseTimeout), "ms! ") + 'Consider increasing the "bidiResponseTimeout" option.'
        ));
        __privateGet(this, _pendingCommands).delete(id);
      }, __privateGet(this, _responseTimeout));
...
var BidiHandler = class extends BidiCore {
   ...
   **/
  async browsingContextStartScreencast(params) {
    const result = await this.send({
   ...
   **/
  async browsingContextStopScreencast(params) {
    const result = await this.send({
...
function initiateBidi(socketUrl, strictSSL = true, userHeaders, responseTimeout) {
...
function hasLoggableBody(body) {
  if (typeof body === "string") {
    return body.length > 0;
  }
  if (body instanceof URLSearchParams || body instanceof FormData) {
    return !body.keys().next().done;
  }

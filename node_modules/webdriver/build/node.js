...
var BidiCore = class {
  ...
  async send(params) {
    ...
    const payload = await new Promise((resolve, reject) => {
      const t = setTimeout(() => {
        reject(new Error(
          `Command ${params.method} with id ${id} timed out after ${this.#responseTimeout}ms! Consider increasing the "bidiResponseTimeout" option.`
        ));
...
async function listWebsocketCandidateUrls(webSocketUrl) {
  ...
  try {
    ...
    if (candidateIps.length > 1) {
      const hostnameMapper = (result) => {
        const candidateUrl = new URL(webSocketUrl);
        candidateUrl.hostname = isIP(result.address) === 6 ? `[${result.address}]` : result.address;
